...
function applyDraftToPage(draft:AIDraft){
 ...
 while((table.tBodies[0]?.rows.length||0)<draft.items.length){
  if(!addButton)throw new Error("Không tìm thấy nút thêm dòng hàng hóa.");
  addButton.click();
 }
 ...
 draft.items.forEach((item,rowIndex)=>{
  const row=rows[rowIndex];if(!row)return;
  const values:Record<keyof typeof indexes,string>={code:item.code||"",category:item.category||"",name:item.name||"",desc:item.desc||"",spec:item.spec||"",unit:item.unit||"",qty:String(item.qty??0),estimate:String(item.estimate??0)};
  (Object.keys(indexes) as Array<keyof typeof indexes>).forEach(key=>{
   const idx=indexes[key];if(idx<0)return;
   const control=row.cells[idx]?.querySelector<HTMLInputElement|HTMLTextAreaElement>("input,textarea");
...
export default function AIPrImporter(){
 const [open,setOpen]=useState(false),[file,setFile]=useState<File|null>(null),[busy,setBusy]=useState(false),[error,setError]=useState(""),[draft,setDraft]=useState<AIDraft|null>(null),[fileName,setFileName]=useState("");
 ...
 useEffect(()=>{
  const mount=()=>{
   const page=document.querySelector<HTMLElement>(".create-page");
   if(!page)return;
   const actions=page.querySelector<HTMLElement>(".heading .actions");
   if(!actions||actions.querySelector(".ai-pr-launch"))return;
   const button=document.createElement("button");
   button.type="button";button.className="ghost ai-pr-launch";button.innerHTML="✦ AI đọc file tạo PR";
   button.addEventListener("click",()=>setOpen(true));
   actions.insertBefore(button,actions.firstChild);
  };
  mount();const observer=new MutationObserver(()=>requestAnimationFrame(mount));observer.observe(document.body,{childList:true,subtree:true});return()=>observer.disconnect();
 },[]);

 const analyze=async()=>{
  if(!file||busy)return;setBusy(true);setError("");setDraft(null);
  try{
   const form=new FormData();form.set("file",file);
   const res=await fetch("/api/ai-pr-import",{method:"POST",body:form});
   const data=await res.json() as {error?:string;draft?:AIDraft;fileName?:string};
   if(!res.ok||!data.draft)throw new Error(data.error||"AI chưa phân tích được file.");
   setDraft(data.draft);setFileName(data.fileName||file.name);
 ...
 const apply=()=>{
  if(!draft)return;
  const hasExisting=Array.from(document.querySelectorAll<HTMLInputElement|HTMLTextAreaElement>(".create-page .draft-table tbody input,.create-page .draft-table tbody textarea")).some(el=>el.value.trim());
  if(hasExisting&&!window.confirm("Bảng PR đang có dữ liệu. Áp dụng AI sẽ ghi đè các dòng đầu tiên. Tiếp tục?"))return;
  try{applyDraftToPage(draft);setOpen(false);}catch(e){setError(e instanceof Error?e.message:"Không thể áp dụng dữ liệu vào PR.")}
 };
 ...
 return <>{open&&<div className="ai-pr-backdrop" onMouseDown={()=>!busy&&setOpen(false)}><div className="ai-pr-modal" onMouseDown={e=>e.stopPropagation()}>
  ...
  {!draft?<div className="ai-pr-upload">
   <input ref={fileRef} type="file" accept=".pdf,.png,.jpg,.jpeg,.webp,.docx,.xlsx,.xls" hidden onChange={e=>{setFile(e.target.files?.[0]||null);setError("")}}/>
   ...
   <div className="ai-pr-actions"><button className="ghost" onClick={()=>setOpen(false)}>Hủy</button><button className="primary" disabled={!file||busy} onClick={()=>void analyze()}>{busy?"AI đang đọc tài liệu...":"✦ Phân tích và tạo PR nháp"}</button></div>
   ...
   <div className="ai-pr-fields">
    <label>Số PR<input value={draft.number} onChange={e=>updateHeader("number",e.target.value)}/></label>
    <label>Ngày PR<input type="date" value={draft.date} onChange={e=>updateHeader("date",e.target.value)}/></label>
    <label>Đơn vị<input value={draft.department} onChange={e=>updateHeader("department",e.target.value)}/></label>
    <label className="wide">Mục đích sử dụng<textarea rows={2} value={draft.purpose} onChange={e=>updateHeader("purpose",e.target.value)}/></label>
   </div>
   ...
   <div className="ai-pr-actions"><button className="ghost" onClick={()=>{setDraft(null);setError("")}}>← Chọn file khác</button><button className="primary" onClick={apply}>Áp dụng vào form PR</button></div>
  </div>}
